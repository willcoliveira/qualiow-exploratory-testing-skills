...
### Added — Knowledge base v0.2.0, v0.3.0 and v0.4.0 (13 → 22 entries)
...
- `data/knowledge/learned-patterns.md` now carries a backend/API/event-driven section, an **HTTP endpoints** subsection, a **triage and re-verification** section ("closed: no legitimate user vector" is a hypothesis to falsify; "partial fix" is the most common re-test outcome; re-verify against a build, not a date), and an expanded set of **disqualifiers** — findings that look real on a first read and do not survive tracing.
