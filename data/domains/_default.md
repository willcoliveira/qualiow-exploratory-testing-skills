# Default Domain: Generic Web Application
...
> Assign a priority tier to every feature before testing. Spend the most time on P0, then P1, and so on. Adjust based on domain-specific risk.

| Tier | Meaning                        | Time Allocation | Criteria                                                                 |
|------|--------------------------------|-----------------|--------------------------------------------------------------------------|
| P0   | Must work perfectly            | ~40% of effort  | Revenue-generating, security-critical, or legally required. Failure = outage or data loss. |
| P1   | Must work correctly            | ~30% of effort  | Core user journeys and frequently used features. Failure = major user frustration. |
| P2   | Should work correctly          | ~20% of effort  | Supporting features that enhance the experience. Failure = degraded but usable. |
| P3   | Nice to have working           | ~10% of effort  | Informational pages, rarely used settings, cosmetic polish. Failure = minor annoyance. |

**How to rank a feature:**
1. Ask: "If this feature is broken, does the business lose money or get sued?" -- If yes, P0.
2. Ask: "If this feature is broken, do users fail their primary task?" -- If yes, P1.
3. Ask: "If this feature is broken, do users notice and complain?" -- If yes, P2.
4. Everything else is P3.

## Completeness Checklist

> Minimum features and states every web application should have. Test for the *existence* of these before deep-diving into any single feature.
...
> After every state-changing action (create, update, delete, submit), verify these:

1. **Persistence** -- Reload the page. Does the change survive? Is the data still correct?
2. **Reflection** -- Does the change appear everywhere it should? (List views, detail views, summaries, exports.)
3. **Consistency** -- Do calculated values (totals, counts, timestamps) update correctly?
4. **Reversibility** -- If the action can be undone (delete, cancel), does the system return to a clean state?
5. **Concurrency** -- If two users (or two tabs) act on the same data, does the system handle it without silent data loss?
6. **Cascade** -- If the changed item is referenced elsewhere (e.g., a category that contains products), are related items updated or handled?

## Cross-Feature Journeys

> Test these generic end-to-end flows that cross multiple features. Bugs often hide at the seams between features.

### Journey 1: New User Onboarding
`Register -> Verify Email -> Login -> Complete Profile -> Use Primary Feature -> Logout`
...
### Journey 2: Content Lifecycle
`Create Item -> View in List -> Edit Item -> Verify Changes -> Delete Item -> Verify Removal`
- Verify: item appears in all relevant views after creation, edits persist after reload, deletion removes item from all views and does not leave orphaned references.

### Journey 3: Error Recovery
`Start Action -> Force Error (disconnect network, submit invalid data) -> See Error State -> Retry -> Complete Successfully`
