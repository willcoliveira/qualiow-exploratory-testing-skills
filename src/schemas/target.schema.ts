...
// never stored here.
export const ApiSurfaceConfigSchema = z
  .object({
    // Defaults to the target's base_url when omitted.
    base_url: z.string().optional(),
    auth: z
      .enum(['session-cookie', 'bearer-env', 'api-key-env', 'none'])
      .optional(),
...
export const SourceBranchConfigSchema = z
  .object({
    repo_path: z.string(),
    branch: z.string(),
...
export const MobileDeviceConfigSchema = z
  .object({
...
export const MobileWebConfigSchema = z
  .object({
...
export const SourceRepoConfigSchema = z
  .object({
    path: z.string(),
...
export const MobileScopeConfigSchema = z
  .object({
...
export const MobileTargetConfigSchema = z
  .object({
    id: z.string().min(1),
    name: z.string().min(1),
...
export type TargetConfigOutput =
  | z.output<typeof WebTargetConfigSchema>
  | z.output<typeof MobileTargetConfigSchema>;

// Discriminate on the presence of `platform` (mobile-only, required there)
// rather than z.union — a union wraps branch failures in one `invalid_union`
// issue and loses the per-field error paths the CLI validate output reports.
// Unlike a bare z.custom(), the transform returns the *parsed* (strict-checked)
// value, so unknown keys are rejected and callers get a validated object.
export const TargetConfigSchema = z.unknown().transform((data, ctx) => {
  if (typeof data !== 'object' || data === null || Array.isArray(data)) {
    ctx.addIssue({ code: 'custom', message: 'Expected a target config object' });
    return z.NEVER;
  }
  const isMobile = 'platform' in (data as Record<string, unknown>);
  ...
  return result.data as TargetConfigOutput;
});
